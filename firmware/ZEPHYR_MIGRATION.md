...
- [ ] Install `west` + Zephyr SDK.
- [ ] Build & flash `samples/hello_world` for `rpi_pico2/rp2350a/m33` on
      actual hardware.
- [ ] Confirm flashing workflow (UF2 drag-drop or `picotool`/SWD).
- [ ] Confirm USB CDC console/logging, matching current [main.c](main.c)
      behavior.
